/**
 * 서버사이드 전용 프롬프트 라이브러리
 * 이 파일은 프론트엔드 번들에 포함되지 않으며, 서버에서만 접근 가능합니다.
 * 
 * 보안 요구사항:
 * - 이 파일의 내용은 클라이언트에 절대 노출되어서는 안됩니다
 * - 환경변수를 통해 프롬프트 변형 가능
 * - 프롬프트 버전 관리 및 A/B 테스트 지원
 */

// 환경변수에서 프롬프트 버전 확인 (기본값: 'standard')
const PROMPT_VERSION = process.env.PROMPT_VERSION || 'standard';

/**
 * 제조 리포트 분석을 위한 핵심 프롬프트
 * 3단계 처리: data_schema → markdown_template → html_template
 */
const ANALYSIS_PROMPT_STANDARD = `
...
/**
 * 프리미엄 사용자용 고급 프롬프트 (더 정확한 분석)
 */
const ANALYSIS_PROMPT_PREMIUM = `
${ANALYSIS_PROMPT_STANDARD}
 ...
 * 사용자 티어에 따른 프롬프트 선택
 */
export function getAnalysisPrompt(userTier: 'free' | 'premium' | 'enterprise' = 'free'): string {
  // 환경변수로 프롬프트 버전 오버라이드 가능
  if (process.env.FORCE_PREMIUM_PROMPT === 'true') {
    return ANALYSIS_PROMPT_PREMIUM;
  }
  ...
  switch (userTier) {
    ...
    case 'free':
    default:
      return ANALYSIS_PROMPT_STANDARD;
  }
}

/**
 * 프롬프트 메타데이터 (모니터링 및 A/B 테스트용)
 */
export function getPromptMetadata() {
  return {
    version: PROMPT_VERSION,
    standardLength: ANALYSIS_PROMPT_STANDARD.length,
    premiumLength: ANALYSIS_PROMPT_PREMIUM.length,
    lastUpdated: '2024-01-20',
    securityLevel: 'SERVER_ONLY'
  };
}
 ...
 * 프롬프트 유효성 검증
 */
export function validatePromptIntegrity(): boolean {
  try {
    // 기본 프롬프트 무결성 검증
    const hasRequiredSections = [
      '1단계 — data_schema',
      '2단계 — design(markdown_template)',
      '3단계 — html_template'
    ].every(section => ANALYSIS_PROMPT_STANDARD.includes(section));

    return hasRequiredSections;
  } catch (error) {
    console.error('프롬프트 무결성 검증 실패:', error);
    return false;
  }
}

// 보안 검증: 이 모듈이 서버사이드에서만 실행되는지 확인
if (typeof window !== 'undefined') {
  throw new Error('🚨 SECURITY VIOLATION: 프롬프트 라이브러리가 클라이언트에서 실행되었습니다!');
}
